...
namespace ET.Server
{
    ...
    public class RobotCase_FirstCase: ARobotCase
    {
        protected override async ETTask Run(RobotCase robotCase)
        {
            ...
            {
                M2C_TestRobotCase response = await robotScene.GetComponent<Client.PlayerSessionComponent>().Session.Call(new C2M_TestRobotCase() {N = robotScene.Zone}) as M2C_TestRobotCase;
