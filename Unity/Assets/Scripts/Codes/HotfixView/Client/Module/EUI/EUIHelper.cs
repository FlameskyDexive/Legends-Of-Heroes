...
namespace ET
{
    public static class EUIHelper
    {
        ...
        #region UI辅助方法
        public static void SetGray(this Image image, bool gray)
        {
            if (null == image)
            {
            ...
            if (gray)
            {
                Material _material = new Material(Shader.Find("UI/ImageGrey"));
        ...
        {
            var oldCount = loopScrollRect.totalCount;
  ...
  #region UI按钮事件
      ...
      { 
        ...
        public static void AddListenerWithParam<T, A, B>(this Button button, Action<T, A, B> clickEventHandler, T param1, A param2, B param3)
        {
            button.onClick.RemoveAllListeners();
            button.onClick.AddListener(() => { clickEventHandler(param1, param2, param3); });
        }

        public static void AddListener(this EUIButton button, UnityAction clickEventHandler)
        {
            button.onClick.RemoveAllListeners();
            button.onClick.AddListener(button.ClickCallBack);
            button.onClick.AddListener(clickEventHandler);
        }
        public static void AddLongClickListener(this EUIButton button, UnityAction<bool> action)
        {
            button.OnLongPress.RemoveAllListeners();
            button.OnLongPress.AddListener((value) => { action(value); });
        }

        public static void AddPointerClickListener(this EUIButton button, UnityAction<PointerEventData> action)
        {
            button.OnPointClick.RemoveAllListeners();
            button.OnPointClick.AddListener((value) => { action(value); });
        }
        ...
        public static void AddListener(this ToggleGroup toggleGroup, UnityAction<int> selectEventHandler)
       {
        ...
        public static void AddListener(this InputField input, Action<string> action)
        {
            input.onValueChanged.RemoveAllListeners();
            input.onValueChanged.AddListener((value) => { action?.Invoke(value); });
        }
