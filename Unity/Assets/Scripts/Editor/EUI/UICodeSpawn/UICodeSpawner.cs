...
public partial class UICodeSpawner
{
    ...
    static void SpawnCodeForDlg(GameObject gameObject)
    {
        ...
        strBuilder.AppendFormat("\t\tpublic static void ShowWindow(this {0} self, ShowWindowDataBase contextData = null)\n", strDlgName);
        strBuilder.AppendLine("\t\t{");
	...
	static void SpawnCodeForDlgEventHandle(GameObject gameObject)
    {
        ...
        strBuilder.AppendFormat("\tpublic  class {0}EventHandler : IAUIEventHandler\r\n", strDlgName);
          ...
          strBuilder.AppendLine("\t\tpublic void OnShowWindow(UIBaseWindow uiBaseWindow, ShowWindowDataBase contextData = null)")
	          .AppendLine("\t\t{");
	...
	static void SpawnCodeForDlgModel(GameObject gameObject)
    {
	    ...
	    strBuilder.AppendLine("\t\tpublic "+strDlgName+"ViewComponent View { get => this.GetParent<UIBaseWindow>().GetComponent<"+ strDlgName +"ViewComponent>();} \r\n");
				...
				foreach (var uiComponent in WidgetInterfaceList)
				{
					...
					if ( Path2WidgetCachedDict.ContainsKey(child.name)  )
					{
                        ...
                        if (component is UnityEngine.UI.Button)
                            tmp = child.GetComponent<EUIButton>();
                        else if (component is UnityEngine.UI.Image)
                            tmp = child.GetComponent<EUIImage>();
                        if (component is UnityEngine.UI.Text)
                            tmp = child.GetComponent<EUIText>();

                        if (!Path2WidgetCachedDict[child.name].Contains(tmp ?? component))
