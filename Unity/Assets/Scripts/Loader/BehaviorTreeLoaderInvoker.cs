...
{
    [Invoke]
    public class GetOneBehaviorTreeBytesInvoker : AInvokeHandler<BehaviorTreeLoader.GetOneBehaviorTreeBytes, ETTask<byte[]>>
    {
        public override async ETTask<byte[]> Handle(BehaviorTreeLoader.GetOneBehaviorTreeBytes args)
        {
        ...
        {
            string assetFilePath = Path.Combine(BehaviorTreeLoader.BehaviorTreeAssetDir, $"{treeName}.asset").Replace("\\", "/");
            ...
            Type assetType = editorAssembly.GetType("ET.BehaviorTreeAsset");
            Type exporterType = editorAssembly.GetType("ET.BehaviorTreeExporter");
            ...
            if (packageObject is not BehaviorTreePackage package)
            {
