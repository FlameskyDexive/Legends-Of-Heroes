...
{
    [Serializable]
    public sealed class BTreeEditorNodeData
    {
        public string NodeId = Guid.NewGuid().ToString("N");
        public string Title = string.Empty;
        public BTreeNodeKind NodeKind = BTreeNodeKind.Action;
        ...
        public List<ET.BTreePatrolPointData> PatrolPoints = new();
        ...
        public BTreeEditorNodeData Clone()
        {
            BTreeEditorNodeData node = new()
            {
                NodeId = this.NodeId,
                Title = this.Title,
            ...
            foreach (ET.BTreePatrolPointData patrolPoint in this.PatrolPoints)
            {
                node.PatrolPoints.Add(patrolPoint?.Clone() ?? new ET.BTreePatrolPointData());
            }
    ...
    public sealed class BTreeAsset : ScriptableObject
    {
        ...
        public static void CreateAsset()
        {
            ...
            BTreeAsset asset = CreateInstance<BTreeAsset>();
            asset.name = System.IO.Path.GetFileNameWithoutExtension(path);
            asset.TreeName = asset.name;
            asset.ExportRelativePath = $"{BTreeBytesLoader.ClientBehaviorTreeBytesDir}/{asset.name}.bytes";
        ...
        public void EnsureInitialized()
        {
            ...
            {
                this.ExportRelativePath = $"{BTreeBytesLoader.ClientBehaviorTreeBytesDir}/{this.TreeName}.bytes";
            }

            foreach (BTreeEditorNodeData node in this.Nodes)
            {
                ...
                {
                    node.Title = BTreeEditorUtility.GetDefaultTitle(node.NodeKind, node.NodeTypeId);
                }
        ...
        public BTreeEditorNodeData GetNode(string nodeId)
        {
            foreach (BTreeEditorNodeData node in this.Nodes)
            {
        ...
        public BTreeEditorNodeData GetRootNode()
        {
            return this.GetNode(this.RootNodeId);
        }

        public BTreeEditorNodeData AddNode(BTreeNodeKind nodeKind, Vector2 position, string nodeTypeId = "")
        {
            BTreeEditorNodeData node = new()
            {
                NodeKind = nodeKind,
                NodeTypeId = nodeTypeId,
                Title = BTreeEditorUtility.GetDefaultTitle(nodeKind, nodeTypeId),
                Position = new Rect(position.x, position.y, 240, 140),
            };

            BTreeEditorUtility.SyncNodeDescriptor(node, true);
        ...
        {
            BTreeEditorNodeData rootNode = this.GetRootNode();
            if (rootNode == null)
            {
                rootNode = new BTreeEditorNodeData()
                {
            ...
            {
                rootNode.NodeKind = BTreeNodeKind.Root;
                rootNode.Title = "Root";
            }
