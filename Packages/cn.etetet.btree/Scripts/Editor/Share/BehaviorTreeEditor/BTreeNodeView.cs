...
namespace ET
{
    public sealed class BTreeNodeView : Node
    {
        private readonly Label summaryLabel;
        private readonly Action onChanged;
        private readonly Action<BTreeNodeView> onSelected;
        private readonly Action<BTreeNodeView> onDoubleClicked;
        private readonly VisualElement topPortContainer;
        private readonly VisualElement bottomPortContainer;

        public BTreeNodeView(BTreeEditorNodeData data, Action<BTreeNodeView> onSelected, Action onChanged,
            Action<BTreeNodeView> onDoubleClicked = null)
        {
            this.Data = data;
            ...
            if (BTreeEditorUtility.HasInputPort(data.NodeKind))
            {
            ...
            if (BTreeEditorUtility.HasOutputPort(data.NodeKind))
            {
                this.OutputPort = this.InstantiatePort(Orientation.Vertical, Direction.Output, BTreeEditorUtility.GetOutputCapacity(data.NodeKind), typeof(bool));
        ...
        public void RefreshView(BTreeNodeState debugState)
        {
            this.title = BTreeEditorUtility.GetNodeTitle(this.Data);
            this.summaryLabel.text = BTreeEditorUtility.GetNodeSummary(this.Data);
            this.titleContainer.style.backgroundColor = BTreeEditorUtility.GetNodeHeaderColor(this.Data.NodeKind, debugState);
