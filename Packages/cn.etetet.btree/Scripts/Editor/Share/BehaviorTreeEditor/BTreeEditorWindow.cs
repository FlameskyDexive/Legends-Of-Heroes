...
namespace ET
{
    public sealed class BTreeEditorWindow : EditorWindow
    {
        ...
        private BTreeNodeView selectedNodeView;
        ...
        private BTreeConnectionStyle connectionStyle = BTreeConnectionStyle.Orthogonal;
        ...
        public static void Open(BTreeAsset asset = null)
        {
            BTreeEditorWindow window = GetWindow<BTreeEditorWindow>();
            window.titleContent = new GUIContent("BehaviourTreeEditor");
            BTreeAsset selectedAsset = asset ?? GetSelectedAsset();
        ...
        {
            if (EditorUtility.InstanceIDToObject(instanceId) is not BTreeAsset asset)
            {
                return false;
            }
        ...
        private static BTreeAsset GetSelectedAsset()
        {
            if (Selection.activeObject is BTreeAsset behaviorTreeAsset)
            {
            ...
            {
                if (selectedObject is BTreeAsset asset)
                {
                    return asset;
                }
            }
        ...
        public BTreeDebugSnapshot GetActiveSnapshot()
        {
            ...
            BTreeDebugHub debugHub = BTreeDebugHub.Instance;
            if (debugHub == null)
            {
                return null;
            }
            ...
            {
                debugHub.Snapshots.TryGetValue(this.selectedRuntimeId, out BTreeDebugSnapshot selectedSnapshot);
        ...
        private static bool IsRuntimeDebugAvailable()
        {
            ...
            return globalConfig.EditorScripts && globalConfig.CodeMode == CodeMode.ClientServer;
        }
        ...
        public void SelectNode(BTreeNodeView nodeView)
        {
            this.selectedNodeView = nodeView;
            this.inspectorPanel?.MarkDirtyRepaint();
        }

        public void OpenNodeScript(BTreeNodeView nodeView)
        {
            ...
            if (BTreeEditorUtility.TryOpenNodeScript(nodeView.Data))
            {
                return;
            }
        ...
        private void OnEnable()
        {
            ...
            this.connectionStyle = (BTreeConnectionStyle)EditorPrefs.GetInt(ConnectionStyleEditorPrefKey, (int)BTreeConnectionStyle.Orthogonal);
        }
        ...
        private void CreateGUI()
        {
            ...
            try
            {
                this.graphView = new BTreeGraphView(this);
            }
            catch (Exception exception)
            {
                Label errorLabel = new($"BTreeGraphView init failed:\n{exception}");
            ...
            if (this.asset == null && Selection.activeObject is BTreeAsset selectedAsset)
            {
        ...
        {
            if (Selection.activeObject is BTreeAsset selectedAsset)
            {
                this.LoadAsset(selectedAsset);
            }
        }
        ...
        private void Update()
        {
            BTreeDebugSnapshot snapshot = this.GetActiveSnapshot();
        ...
        private Toolbar CreateToolbar()
        {
            ...
            this.toolbar.Add(this.CreateToolbarButton("New", BTreeAsset.CreateAsset));
            ...
            lineMenu.menu.AppendAction("Straight", _ => this.SetConnectionStyle(BTreeConnectionStyle.Straight),
                _ => this.connectionStyle == BTreeConnectionStyle.Straight ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
            lineMenu.menu.AppendAction("Orthogonal", _ => this.SetConnectionStyle(BTreeConnectionStyle.Orthogonal),
                _ => this.connectionStyle == BTreeConnectionStyle.Orthogonal ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
            lineMenu.menu.AppendAction("Curve", _ => this.SetConnectionStyle(BTreeConnectionStyle.Curve),
                _ => this.connectionStyle == BTreeConnectionStyle.Curve ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
        ...
        private void SetConnectionStyle(BTreeConnectionStyle style)
        {
        ...
        private string GetConnectionStyleLabel()
        {
            ...
            {
                BTreeConnectionStyle.Straight => "Line: Straight",
                BTreeConnectionStyle.Curve => "Line: Curve",
                _ => "Line: Orthogonal",
            };
        }
        ...
        private void LoadAsset(BTreeAsset newAsset)
        {
            this.asset = newAsset;
        ...
        private ToolbarMenu CreateAssetMenu()
        {
            ...
            string[] guids = AssetDatabase.FindAssets("t:BTreeAsset");
            foreach (string guid in guids)
            {
                string path = AssetDatabase.GUIDToAssetPath(guid);
                BTreeAsset behaviorTreeAsset = AssetDatabase.LoadAssetAtPath<BTreeAsset>(path);
        ...
        private void DrawInspectorPanel()
        {
            ...
            BTreeAsset selectedAsset = (BTreeAsset)EditorGUILayout.ObjectField("Asset", this.asset, typeof(BTreeAsset), false);
        ...
        private void DrawNodeInspector(BTreeEditorNodeData node)
        {
            BTreeEditorUtility.SyncNodeDescriptor(node);
            EditorGUILayout.LabelField("Script", BTreeEditorUtility.GetNodeScriptName(node));
            ...
            {
                case BTreeNodeKind.Action:
                    if (BTreeEditorUtility.TryGetDescriptor(node, out ABTreeNodeDescriptor actionDescriptor))
                    {
                    ...
                    {
                        DrawHandlerPopup("Handler", BTreeEditorUtility.GetActionHandlerNames(), node, value => node.HandlerName = value);
                        DrawArguments(node);
                    }
                    break;
                case BTreeNodeKind.Condition:
                    if (BTreeEditorUtility.TryGetDescriptor(node, out ABTreeNodeDescriptor conditionDescriptor))
                    {
                        this.DrawDescriptorNode(node, conditionDescriptor);
                    }
                    else
                    {
                        DrawHandlerPopup("Handler", BTreeEditorUtility.GetConditionHandlerNames(), node, value => node.HandlerName = value);
                        DrawArguments(node);
                    }
                    break;
                case BTreeNodeKind.Service:
                    if (BTreeEditorUtility.TryGetDescriptor(node, out ABTreeNodeDescriptor serviceDescriptor))
                    {
                        this.DrawDescriptorNode(node, serviceDescriptor, true);
                    }
                    else
                    {
                        DrawHandlerPopup("Handler", BTreeEditorUtility.GetServiceHandlerNames(), node, value => node.HandlerName = value);
                        node.IntervalMilliseconds = EditorGUILayout.IntField("Interval (ms)", node.IntervalMilliseconds);
                        DrawArguments(node);
                    }
                    break;
                case BTreeNodeKind.Wait:
                    node.WaitMilliseconds = EditorGUILayout.IntField("Delay (ms)", node.WaitMilliseconds);
                    break;
                case BTreeNodeKind.Repeater:
                    node.MaxLoopCount = EditorGUILayout.IntField("Repeat Count", node.MaxLoopCount);
                    break;
                case BTreeNodeKind.BlackboardCondition:
                    DrawBlackboardKeyPopup(node);
                    node.CompareOperator = (BTreeCompareOperator)EditorGUILayout.EnumPopup("Operator", node.CompareOperator);
                    node.AbortMode = (BTreeAbortMode)EditorGUILayout.EnumFlagsField("Abort Mode", node.AbortMode);
                    DrawSerializedValueEditor("Compare Value", node.CompareValue);
                    break;
                case BTreeNodeKind.Parallel:
                    node.SuccessPolicy = (BTreeParallelPolicy)EditorGUILayout.EnumPopup("Success Policy", node.SuccessPolicy);
                    node.FailurePolicy = (BTreeParallelPolicy)EditorGUILayout.EnumPopup("Failure Policy", node.FailurePolicy);
                    break;
                case BTreeNodeKind.SubTree:
                    node.SubTreeAsset = (BTreeAsset)EditorGUILayout.ObjectField("SubTree", node.SubTreeAsset, typeof(BTreeAsset), false);
        ...
        private void DrawBlackboardPanel()
        {
            ...
            {
                BTreeBlackboardEntryData entry = this.asset.BlackboardEntries[index];
                ...
                entry.ValueType = (BTreeValueType)EditorGUILayout.EnumPopup("Type", entry.ValueType);
            ...
            {
                this.asset.BlackboardEntries.Add(new BTreeBlackboardEntryData());
            }
            ...
            if (!IsRuntimeDebugAvailable())
            {
                EditorGUILayout.HelpBox("Runtime debug is only available when CodeMode is ClientServer and EditorScripts is enabled.", MessageType.None);
            }
            else
            {
                BTreeDebugHub debugHub = BTreeDebugHub.Instance;
                List<BTreeDebugSnapshot> snapshots = debugHub != null
        ...
        private void DrawRuntimeBlackboard(BTreeDebugSnapshot snapshot)
        {
            if (snapshot == null)
            {
            ...
            foreach (BTreeBlackboardEntryData entry in this.asset.BlackboardEntries)
            {
                ...
                string currentValue = snapshot.BlackboardValues.TryGetValue(entry.Key, out string runtimeValue)
                        ? runtimeValue
                        : BTreeValueUtility.ToDisplayString(entry.DefaultValue);
        ...
        private void DrawBlackboardKeyPopup(BTreeEditorNodeData node)
        {
        ...
        private void ExportCurrentAsset()
        {
            ...
            try
            {
                BTreeExporter.BTreeExportResult result = BTreeExporter.ExportToFiles(this.asset);
        ...
        private void DrawDescriptorNode(BTreeEditorNodeData node, ABTreeNodeDescriptor descriptor, bool drawInterval = false)
        {
            if (descriptor == null)
            {
                return;
        ...
        private void DrawPatrolPointsEditor(BTreeEditorNodeData node)
        {
            EditorGUILayout.Space(6);
            EditorGUILayout.LabelField("Patrol Points", EditorStyles.boldLabel);
            for (int index = 0; index < node.PatrolPoints.Count; ++index)
            {
                ET.BTreePatrolPointData patrolPoint = node.PatrolPoints[index] ?? new ET.BTreePatrolPointData();
            ...
            {
                node.PatrolPoints.Add(new ET.BTreePatrolPointData());
            }
        }

        private void DrawDescriptorArguments(BTreeEditorNodeData node, IReadOnlyList<BTreeNodeParameterDefinition> parameters)
        {
        ...
        private void DrawDescriptorArgument(BTreeEditorNodeData node, BTreeNodeParameterDefinition parameter)
        {
            BTreeArgumentData argument = BTreeEditorUtility.GetOrCreateArgument(node, parameter);
            ...
            {
                case BTreeNodeParameterEditorHint.BlackboardKey:
                    this.DrawBlackboardKeyArgument(label, argument);
                    break;
                case BTreeNodeParameterEditorHint.CompareOperator:
                    argument.Value.ValueType = BTreeValueType.Integer;
                    BTreeCompareOperator compareOperator = (BTreeCompareOperator)argument.Value.IntValue;
                    compareOperator = (BTreeCompareOperator)EditorGUILayout.EnumPopup(label, compareOperator);
                    argument.Value.IntValue = (int)compareOperator;
                    break;
                case BTreeNodeParameterEditorHint.MultilineText:
                    argument.Value.ValueType = BTreeValueType.String;
        ...
        private void DrawBlackboardKeyArgument(string label, BTreeArgumentData argument)
        {
            argument.Value ??= new BTreeSerializedValue();
            argument.Value.ValueType = BTreeValueType.String;
            string[] options = this.GetBlackboardKeyOptions();
            if (options.Length == 0)
            {
        ...
        private static void DrawArguments(BTreeEditorNodeData node)
        {
            ...
            {
                node.Arguments.Add(new BTreeArgumentData());
            }
        }

        private static void DrawHandlerPopup(string label, string[] options, BTreeEditorNodeData node, Action<string> assign)
        {
        ...
        private static void DrawSerializedValueEditor(string label, BTreeSerializedValue value, BTreeValueType forceType = BTreeValueType.None)
        {
            if (value == null)
            {
                return;
            ...
            value.ValueType = forceType == BTreeValueType.None
                    ? (BTreeValueType)EditorGUILayout.EnumPopup(label + " Type", value.ValueType)
                    : forceType;
