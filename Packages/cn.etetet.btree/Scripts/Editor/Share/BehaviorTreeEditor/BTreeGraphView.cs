...
namespace ET
{
    public sealed class BTreeGraphView : GraphView
    {
        private sealed class ClipboardSelection
        {
            public readonly List<BTreeEditorNodeData> Nodes = new();
            public readonly List<(string ParentId, string ChildId)> Connections = new();
        }

        private static ClipboardSelection clipboard;
        private readonly BTreeConnectionLayer connectionLayer;
        private readonly Dictionary<string, BTreeNodeView> nodeViews = new();
        private readonly BTreeEditorWindow window;
        private readonly BTreeGridBackground gridBackground;
        private readonly BTreeSearchWindowProvider searchWindowProvider;
        private bool isPopulating;
        private BTreeMiniMap miniMap;
        ...
        public BTreeGraphView(BTreeEditorWindow window)
        {
            this.window = window;
            this.style.flexGrow = 1;
            this.style.backgroundColor = new Color(0.12f, 0.12f, 0.12f);

            this.gridBackground = new BTreeGridBackground(this);
            ...
            this.connectionLayer = new BTreeConnectionLayer(this);
            ...
            this.searchWindowProvider = ScriptableObject.CreateInstance<BTreeSearchWindowProvider>();
        ...
        public BTreeAsset Asset { get; private set; }

        public BTreeEditorWindow GetWindow()
        {
            return this.window;
        }
        ...
        public void PopulateView(BTreeAsset asset)
        {
            ...
            foreach (BTreeEditorNodeData node in asset.Nodes)
            {
                if (!this.nodeViews.TryGetValue(node.NodeId, out BTreeNodeView parentView) || parentView.OutputPort == null)
                {
                    continue;
                }
                ...
                {
                    if (!this.nodeViews.TryGetValue(childId, out BTreeNodeView childView) || childView.InputPort == null)
                    {
                        continue;
                    }
        ...
        public void CreateNode(BTreeNodeKind nodeKind, Vector2 localMousePosition, string nodeTypeId = "")
        {
            if (this.Asset == null)
            {
                return;
        ...
        public void CreateNodeAtContentPosition(BTreeNodeKind nodeKind, Vector2 contentPosition, string nodeTypeId = "")
        {
            if (this.Asset == null)
            {
                return;
            ...
            BTreeEditorNodeData node = this.Asset.AddNode(nodeKind, contentPosition, nodeTypeId);
            this.AddNodeView(node);
        ...
        public void PasteNodes(Vector2? centerPosition = null)
        {
            ...
            foreach (BTreeEditorNodeData clipboardNode in clipboard.Nodes)
            {
            ...
            Dictionary<string, BTreeEditorNodeData> pastedNodes = new();
            foreach (BTreeEditorNodeData clipboardNode in clipboard.Nodes)
            {
                BTreeEditorNodeData pastedNode = clipboardNode.Clone();
            ...
            {
                if (!pastedNodes.TryGetValue(parentId, out BTreeEditorNodeData parentNode) || !pastedNodes.TryGetValue(childId, out BTreeEditorNodeData childNode))
                {
                    continue;
                }
            ...
            this.ClearSelection();
            foreach (BTreeEditorNodeData pastedNode in pastedNodes.Values)
            {
                if (this.nodeViews.TryGetValue(pastedNode.NodeId, out BTreeNodeView pastedView))
                {
                    this.AddToSelection(pastedView);
                }
            }

            if (pastedNodes.Values.FirstOrDefault() is BTreeEditorNodeData firstPastedNode && this.nodeViews.TryGetValue(firstPastedNode.NodeId, out BTreeNodeView firstPastedView))
            {
        ...
        {
            foreach (BTreeNodeView nodeView in this.nodeViews.Values)
            {
                nodeView.RefreshView(BTreeNodeState.Inactive);
            }
        ...
        public void AutoLayoutTree()
        {
            ...
            float Measure(string nodeId)
            {
                ...
                BTreeEditorNodeData node = this.Asset.GetNode(nodeId);
            ...
            foreach (BTreeEditorNodeData node in this.Asset.Nodes)
            {
                if (connectedNodeIds.Contains(node.NodeId) || node.NodeKind == BTreeNodeKind.Root)
                {
                    continue;
                }
        ...
        {
            List<BTreeNodeView> selectedNodeViews = this.GetSelectedNodeViews();
        ...
        {
            List<BTreeNodeView> selectedNodeViews = this.GetSelectedNodeViews().OrderBy(view => view.GetPosition().center.x).ToList();
        ...
        {
            List<BTreeNodeView> selectedNodeViews = this.GetSelectedNodeViews().OrderBy(view => view.GetPosition().center.y).ToList();
            if (selectedNodeViews.Count < 3)
            {
                return;
            }
        ...
        public void SetConnectionStyle(BTreeConnectionStyle style)
        {
        ...
        public BTreeConnectionStyle GetConnectionStyle()
        {
        ...
        public void SelectNode(string nodeId)
        {
            ...
            if (!this.nodeViews.TryGetValue(nodeId, out BTreeNodeView nodeView))
            {
                return;
            }
        ...
        public void RefreshDebugStates(BTreeDebugSnapshot snapshot)
        {
            foreach (BTreeNodeView nodeView in this.nodeViews.Values)
            {
                BTreeNodeState state = BTreeNodeState.Inactive;
                if (snapshot != null && snapshot.NodeStates.TryGetValue(nodeView.Data.NodeId, out BTreeNodeState debugState))
                {
                    state = debugState;
                }
        ...
        public override void BuildContextualMenu(ContextualMenuPopulateEvent evt)
        {
            ...
            BTreeNodeView contextNodeView = this.GetContextNodeView(evt);
        ...
        private void AddNodeView(BTreeEditorNodeData node)
        {
            BTreeNodeView view = new(node, this.window.SelectNode, () =>
            {
        ...
        private void OpenNodeScript(BTreeNodeView nodeView)
        {
            if (nodeView == null)
            {
                return;
        ...
        private bool CanOpenNodeScript(BTreeNodeView nodeView)
        {
            return nodeView != null && BTreeEditorUtility.TryGetNodeScriptAsset(nodeView.Data, out _);
        }

        private BTreeNodeView GetContextNodeView(ContextualMenuPopulateEvent evt)
        {
            if (evt?.target is BTreeNodeView directNodeView)
            {
            ...
            {
                BTreeNodeView ancestorNodeView = visualElement.GetFirstAncestorOfType<BTreeNodeView>();
        ...
        private GraphViewChange OnGraphViewChanged(GraphViewChange graphViewChange)
        {
            ...
            if (graphViewChange.elementsToRemove != null)
            {
                foreach (GraphElement element in graphViewChange.elementsToRemove)
                {
                    ...
                    if (element is BTreeNodeView nodeView)
                    {
        ...
        {
            List<BTreeNodeView> selectedNodeViews = this.selection.OfType<BTreeNodeView>().Where(view => BTreeEditorUtility.CanDelete(view.Data)).ToList();
            ...
            foreach (BTreeNodeView nodeView in selectedNodeViews)
            {
        ...
        private void ApplyNodePositions(List<BTreeNodeView> nodeViewsToMove, Func<BTreeNodeView, Rect> positionFactory, string undoName)
        {
            ...
            foreach (BTreeNodeView nodeView in nodeViewsToMove)
            {
        ...
        private List<BTreeNodeView> GetSelectedNodeViews()
        {
            return this.selection.OfType<BTreeNodeView>().ToList();
        }

        private void DeleteSelectionNodes()
        {
            List<GraphElement> elements = this.selection
                .Where(element => element is BTreeNodeView nodeView ? BTreeEditorUtility.CanDelete(nodeView.Data) : element is Edge)
                .Cast<GraphElement>()
                .ToList();
        ...
        {
            return this.selection.OfType<BTreeNodeView>().Any(view => BTreeEditorUtility.CanDelete(view.Data));
        }
        ...
        {
            return this.selection.Any(element => element is Edge || element is BTreeNodeView nodeView && BTreeEditorUtility.CanDelete(nodeView.Data));
        }
        ...
        private void EnsureMiniMap()
        {
            ...
            this.miniMap = new BTreeMiniMap(this);
        ...
        {
            if (edge.output?.node is not BTreeNodeView parentView || edge.input?.node is not BTreeNodeView childView)
            {
                return;
            }
        ...
        public IEnumerable<BTreeNodeView> GetNodeViews()
        {
