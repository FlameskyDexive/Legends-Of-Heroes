...
namespace ET
{
    public enum BTreeConnectionStyle
    {
        Straight = 0,
        Orthogonal = 1,
        Curve = 2,
    ...
    public sealed class BTreeConnectionLayer : VisualElement
    {
        ...
        private void OnGenerateVisualContent(MeshGenerationContext context)
        {
            ...
            foreach (Edge edge in this.graphView.edges.ToList())
            {
                ...
                if (edge.output.node is not BTreeNodeView outputNodeView || edge.input.node is not BTreeNodeView inputNodeView)
                {
                    continue;
                }
                ...
                if (this.connectionStyle == BTreeConnectionStyle.Straight)
                {
                    DrawStraightLine(painter, from, to);
                }
                else if (this.connectionStyle == BTreeConnectionStyle.Curve)
                {
