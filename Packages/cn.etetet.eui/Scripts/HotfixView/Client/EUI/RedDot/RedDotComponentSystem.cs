...
namespace ET.Client
{
    ...
    {
        private const string REDDOT_POOL_NAME = "RedDot";
        ...
        public static void RemoveRedDotNode(this RedDotComponent self, string target)
        {
            ...
            if (!string.IsNullOrEmpty(parent) && self.RedDotNodeParentsDict.TryGetValue(parent, out ListComponent<string> list))
            {
                list.Remove(target);
                if (list.Count <= 0)
                {
        ...
        private static void UpdateLogicNodeRetainCount(this RedDotComponent self, string target, bool isRaiseRetainCount = true)
        {
            ...
            else
            {
                if (self.RedDotNodeRetainCount[target] != 1)
                {
                    Log.Error($"redDot logic node {target} is not show, RetainCount={self.RedDotNodeRetainCount[target]}");
                    return;
                }
                self.RedDotNodeRetainCount[target] += -1;
            }
            ...
            if (curr < 0 || curr > 1)
            {
                Log.Error("count error, redDot node logic error!");
                return;
            }
            ...
            while (isParentExist)
            {
                ...
                {
                    if (self.RedDotMonoViewDict.TryGetValue(parent, out redDotMonoView) && !redDotMonoView.isRedDotActive)
                    {
                        redDotMonoView.Show(self.GetRedDotGameObjectFromPool());
                    }
                }
        ...
        public static void RefreshRedDotViewCount(this RedDotComponent self, string target, int count)
        {
            ...
            while (isParentExist)
            {
                ...
                if (self.RedDotMonoViewDict.TryGetValue(parent, out redDotMonoView) && self.RedDotNodeNeedShowSet.Contains(parent))
                {
        ...
        public static async ETTask PreLoadGameObject(this RedDotComponent self, string assetLocation)
        {
            await GameObjectPoolHelper.InitPoolWithPathAsync(self.Scene<Scene>(), REDDOT_POOL_NAME, assetLocation, 5);
        }

        public static GameObject GetRedDotGameObjectFromPool(this RedDotComponent self)
        {
            return GameObjectPoolHelper.GetObjectFromPool(REDDOT_POOL_NAME, true);
        }
